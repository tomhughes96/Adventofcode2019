def jumble(w1,w2):
	wire1 = [[0,0]]  # list of coordinates of wire1
	wire2 = [[0,0]]  # list of coordinates of wire2
	w1 = w1.split (",")
	w2 = w2.split (",")
	w1d = [] #wire1 vertical parts
	w1l = [] #wire1 horizontal parts
	w2d = [] #wire2 vertical parts
	w2l = [] #wire2 horizontal parts
	cross = [] #where wires cross
	
	for i in range(0,len(w1)): #plot coordinates of wire1
		w1d.append(w1[i][0])
		w1l.append(w1[i][1:len(w1[i])])

	for i in range(0,len(w1)): #plot coordinates of wire1
		if w1d[i] == 'R':
			for j in range(0, int(w1l[i])):
				wire1.append([wire1[len(wire1)-1][0]+1,wire1[len(wire1)-1][1]]) 
		if w1d[i] == 'L':
			for j in range(0, int(w1l[i])):
				wire1.append([wire1[len(wire1)-1][0]-1,wire1[len(wire1)-1][1]]) 
		if w1d[i] == 'U':
			for j in range(0, int(w1l[i])):
				wire1.append([wire1[len(wire1)-1][0],wire1[len(wire1)-1][1]+1])
		if w1d[i] == 'D':
			for j in range(0, int(w1l[i])):
				wire1.append([wire1[len(wire1)-1][0],wire1[len(wire1)-1][1]-1]) 

	for i in range(0,len(w2)): #plot coordinates of wire2
		w2d.append(w2[i][0])
		w2l.append(w2[i][1:len(w2[i])])

	for i in range(0,len(w2)): #plot coordinates of wire2
		if w2d[i] == 'R':
			for j in range(0, int(w2l[i])):
				wire2.append([wire2[len(wire2)-1][0]+1,wire2[len(wire2)-1][1]]) 
		if w2d[i] == 'L':
			for j in range(0, int(w2l[i])):
				wire2.append([wire2[len(wire2)-1][0]-1,wire2[len(wire2)-1][1]]) 
		if w2d[i] == 'U':
			for j in range(0, int(w2l[i])):
				wire2.append([wire2[len(wire2)-1][0],wire2[len(wire2)-1][1]+1])
		if w2d[i] == 'D':
			for j in range(0, int(w2l[i])):
				wire2.append([wire2[len(wire2)-1][0],wire2[len(wire2)-1][1]-1])
	...
	x = 0 #xcoord
	y = 0 #ycoord
	D = [] #minimum distance to closest crossover
	md = len(wire1) + len(wire2) #distance in manhattan metric

	for i in wire1: #part 2
		x = wire1.index(i)+1
		if x < md and i in wire2:
			y = wire2.index(i)+1
			md = min(x + y, md)
	print(md)

	
	md = len(wire1)+len(wire2) #limiter

	for i in wire1: #part 1
		if abs(i[0])+abs(i[1]) < md and i in wire2:
			cross.append(i)
			md = abs(i[0])+abs(i[1])
	for i in cross:
